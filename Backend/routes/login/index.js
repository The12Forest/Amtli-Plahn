...
router.use("/save", (req, res) => {
  ...
  fs.writeFileSync("./Backend/saves/passwords.json", buffer)
    //   console.log(Time() + logprefix + "Passwords saved:     " + buffer)
  console.log(Time() + logprefix + "Passwords saved:     " + '"Hidden"')
  res.send("Passwords Saved")
})

router.use("/load", (req, res) => {
  passwords = JSON.parse(fs.readFileSync("./Backend/saves/passwords.json"))
  // console.log(Time() + logprefix + "Passwords loaded:  " + JSON.stringify(passwords))
  console.log(Time() + logprefix + "Passwords loaded:     " + '["Hidden"]')
...
router.get("/create/:passwdold/:passwd", (req, res) => {
  ...
  if (passwdIndex !== -1) {
    ...
    if (userid == -1) {
      ...
      console.log(Time() + logprefix + "Creating password for user: " + passwords.indexOf(req.params.passwdold))
      res.send("Password was added.")
    } else { 
      console.log(Time() + logprefix + "Creating password: But the password: " + req.params.passwd + " already exists.")
      res.send("Password dose already exist.")
    }
  } else {
    console.log(Time() + logprefix + "Admin user tried to create new Admin but the username/password was wrong.");
...
router.get("/delete/:passwd", (req, res) => {
  if (passwords.length > 1) {
    let userid = passwords.indexOf(req.params.passwd)
    if (userid !== -1) {
      console.log(Time() + logprefix + "Deleting password for user: " + passwords.indexOf(req.params.passwdold))
      passwords.splice(userid)
      res.json({"ok": true, "successful": true})
    } else { 
      console.log(Time() + logprefix + "Deleting password: But the password: " + req.params.passwd + " dose not exists.")
...
router.get("/update/:passwdold/:passwd", (req, res) => {
  ...
  if (userid == -1) {
    console.log(Time() + logprefix + "Updating password: " + req.params.passwd + " but password dose not exist.")
    res.send("Password was not updated because it does not exist.")
  } else { 
    console.log(Time() + logprefix + "Updating password from user: " + passwords.indexOf(req.params.passwd))
...
router.get("/:passwd", (req, res) => {
  ...
  if (passwdIndex !== -1) {
    console.log(Time() + logprefix + "Admin user: " + passwdIndex + " logged in.");
    res.sendFile(path.resolve(__dirname, adminPanelPath));
  } else {
    console.log(Time() + logprefix + "Admin user tried to login but the username/password was wrong.");
    res.status(401).send("Unauthorized"); // optional: mit Statuscode
  }
});
