...
router.use("/save", (req, res) => {
  ...
  console.log(Time() + logprefix + "Settings saved:         " + buffer)
  res.json(buffer)
})

router.use("/load", (req, res) => {
  time = JSON.parse(fs.readFileSync("./Backend/saves/time.json"))
  console.log(Time() + logprefix + "Times loaded:         "+ JSON.stringify(time))
  res.json(time)
})
...
router.use("/set/:userName/:time", (req, res) => {
  ...
  if (usertime == -1) {
    ...
    usertime = parseInt(usertime)
    console.log(Time() + logprefix + "User " + req.params.userName + " created.")
  }
  usertime = usertime + 1
  time[usertime] = req.params.time
  res.status(200).json({"ok": true, "username": req.params.userName, "time": time[usertime]})
  console.log(Time() + logprefix + "Time update: User "+ req.params.userName + " to " + time[usertime])
})
...
router.use("/:userName", (req, res) => {
  ...
  if (usertime !== -1) {
    usertime = usertime + 1
    res.status(200).json({"ok": true, "username": req.params.userName, "time": time[usertime]})
    console.log(Time() + logprefix + "Searched for UserTime User: " + req.params.userName + " Time: " + time[usertime])
  } else {
    res.status(404).json({"ok": false, "username": req.params.userName, "time": "UserNotFound"})
    console.log(Time() + logprefix + "Searched for UserTime User: " + req.params.userName + " but he doesn't exist")
  }
})
